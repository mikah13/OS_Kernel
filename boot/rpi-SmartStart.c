...
bool gpio_edgeDetect(uint_fast8_t gpio, bool lifting, bool Async)
{
	...
	{ // Lifting edge detect
		if (Async)
			GPIO->GPAREN[gpio / 32] = bit; // Asynchronous lifting edge detect register bit set
		else
			GPIO->GPREN[gpio / 32] = bit; // Synchronous lifting edge detect register bit set
	}
	else
	{ // Falling edge detect
		if (Async)
			GPIO->GPAFEN[gpio / 32] = bit; // Asynchronous falling edge detect register bit set
		else
			GPIO->GPFEN[gpio / 32] = bit; // Synchronous falling edge detect register bit set
	}
	return true; // Return true
}
...
bool gpio_fixResistor(uint_fast8_t gpio, GPIO_FIX_RESISTOR resistor)
{
	...
	if (gpio > 54)
		return false; // Check GPIO pin number valid, return false if invalid
	if (resistor < 0 || resistor > PULLDOWN)
		return false;					// Check requested resistor is valid, return false if invalid
	GPIO->GPPUD = resistor;				// Set fixed resistor request to PUD register
	endTime = timer_getTickCount() + 2; // We want a 2 usec delay
	while (timer_getTickCount() < endTime)
	{
...
bool set_Activity_LED(bool on)
{
	...
	{
	case 0xb76:
	{ // arm1176jzf-s AKA Pi1
		if (!ModelCheckHasRun)
		{ // Just check board isn't early Pi1 on GPIO16
			uint32_t model[4];
									...
									MAILBOX_TAG_GET_BOARD_REVISION, 4, 0, 0))
			{
				...
				{						   // Models A, B return 0002 to 000F
					...
					ActivityGPIOPort = 47; // GPIO port 47 activity as default
			}
		}
...
{
	if (console.curPos.y >= GetConsole_Height())
	{
		ClearScreen();
	}
	switch (Ch)
			  ...
			  POINT *lpPoint)
{
	...
	{ // Pointer valid
		if (lpPoint)
			*lpPoint = intDC->curPos; // Return current position
			...
			int nYEnd)
{
	...
	{ // Pointer valid
		...
		if (dx == 0)
			intDC->VertLine(intDC, dy, ydir);
		else if (dy == 0)
			intDC->HorzLine(intDC, dx, xdir);
		else
			intDC->DiagLine(intDC, dx, dy, xdir, ydir);
		return TRUE;
	}
	return FALSE;
}
...
{
	if ((cchString) && (lpString))
	{ // Check text data valid
...
static void VertLine16(INTDC *dc, uint_fast32_t cy, int_fast8_t dir)
{
	...
	{						  // For each y line
		...
		if (dir == 1)
			video_wr_ptr += dc->wth; // Positive offset to next line
		else
			video_wr_ptr -= dc->wth; // Negative offset to next line
	}
...
static void DiagLine16(INTDC *dc, uint_fast32_t dx, uint_fast32_t dy, int_fast8_t xdir, int_fast8_t ydir)
{
	...
	if (dy > eulerMax)
		dy = eulerMax; // If dy greater change to that value
...
static void WriteChar16(INTDC *dc, uint8_t Ch)
{
	...
	for (uint_fast32_t y = 0; y < 4; y++)
	{
		...
		{					   // For each bit
			...
			if (xoffs == 7)
				video_wr_ptr += dc->wth; // If was bit 7 next line down
		}
	}
...
static void PutImage16(INTDC *dc, uint_fast32_t dx, uint_fast32_t dy, IMAGE_PTR ImageSrc, bool BottomUp)
{
	...
	{ // For each line
		...
		if (BottomUp)
			video_wr_ptr.ptrRGB565 -= dc->wth; // Next line up
		else
			video_wr_ptr.ptrRGB565 += dc->wth; // Next line down
	}
}
...
static void WriteChar24(INTDC *dc, uint8_t Ch)
{
	...
	for (uint_fast32_t y = 0; y < 4; y++)
	{
		...
		{							   // For each bit
			...
			int xoffs = i % 8;		   // X offset
			if ((b & 0x80000000) != 0)
				col = dc->TxtColor.rgb; // If bit set take text colour
			video_wr_ptr[xoffs] = col;  // Write pixel
			b <<= 1;					// Roll font bits left
			if (xoffs == 7)
				video_wr_ptr += dc->wth; // If was bit 7 next line down
...
static void PutImage24(INTDC *dc, uint_fast32_t dx, uint_fast32_t dy, IMAGE_PTR ImageSrc, bool BottomUp)
{
	...
	{ // For each line
		...
		if (BottomUp)
			video_wr_ptr.ptrRGB -= dc->wth; // Next line up
		else
			video_wr_ptr.ptrRGB += dc->wth; // Next line down
	}
}
...
static void WriteChar32(INTDC *dc, uint8_t Ch)
{
	...
	for (uint_fast32_t y = 0; y < 4; y++)
	{
		...
		{								// For each bit
			...
			if ((b & 0x80000000) != 0)
				col = dc->TxtColor; // If bit set take text colour
			//video_wr_ptr[xoffs] = col;								// Write pixel
			b <<= 1; // Roll font bits left
			if (xoffs == 7)
				video_wr_ptr += dc->wth; // If was bit 7 next line down
...
static void PutImage32(INTDC *dc, uint_fast32_t dx, uint_fast32_t dy, IMAGE_PTR ImageSrc, bool BottomUp)
{
	...
	{ // For each line
		...
		if (BottomUp)
			video_wr_ptr.ptrRGBA -= dc->wth; // Next line up
		else
			video_wr_ptr.ptrRGBA += dc->wth; // Next line down
	}
}
...
bool PiConsole_Init(int Width, int Height, int Depth, printhandler prn_handler)
{
	uint32_t buffer[19];
	if ((Width == 0) || (Height == 0))
	{ // Has auto width or heigth been requested
		if (mailbox_tag_message(&buffer[0], 5,
								MAILBOX_TAG_GET_PHYSICAL_WIDTH_HEIGHT,
								8, 0, 0, 0))
		{ // Get current width and height of screen
			if (Width == 0)
				Width = buffer[3]; // Width passed in as zero set set current screen width
			if (Height == 0)
				Height = buffer[4]; // Height passed in as zero set set current screen height
		}
		else
			return false; // For some reason get screen physical failed
	}
	if (Depth == 0)
	{ // Has auto colour depth been requested
		...
		{					   // Get current colour depth of screen
			...
			return false; // For some reason get screen depth failed
	}
